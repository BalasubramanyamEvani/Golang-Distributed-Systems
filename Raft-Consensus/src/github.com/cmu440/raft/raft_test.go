...
func TestReElectionHidden2A(t *testing.T) {
	...
	fmt.Printf("Test (2A): Re-election Hidden 2A\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()
	...
	fmt.Println("Reconnecting old leader")
	cfg.connect(leader1)
	...
	// Disconnecting leader and two peers
	fmt.Println("Disconnecting leader and two peers")
	...
	fmt.Println("Checking if there is no leader")
	cfg.checkNoLeader()
...
func TestSmallPartitionConsensusHidden2A(t *testing.T) {
	...
	fmt.Printf("Test (2A): Test Small Parition Consensus 2A\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()

	// forming one partition with one leader and a node
	fmt.Println("Forming partition 2 with leader and 1 node")
	partition2 := make(IntSet)
	partition2[leader1] = struct{}{}
	partition2[(leader1+1)%servers] = struct{}{}
	cfg.disconnect_partition(partition2)

	// waiting for a bit
	time.Sleep(1000 * time.Millisecond)

	// checking for a new leader in original partition
	fmt.Println("Checking for a new leader in original partition")
	cfg.checkOneLeader()

	// checking for previous leader in partition 2
	fmt.Println("Checking for previous leader in partition 2")
...
func TestSmallPartitionNoConsensusHidden2A(t *testing.T) {
	...
	fmt.Printf("Test (2A): Test Small Parition No Consensus 2A\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()

	// forming one partition with one leader and a node
	fmt.Println("Forming partition 2 with leader and 2 nodes")
	partition2 := make(IntSet)
	partition2[leader1] = struct{}{}
	partition2[(leader1+1)%servers] = struct{}{}
	partition2[(leader1+2)%servers] = struct{}{}
	cfg.disconnect_partition(partition2)
	...
	// checking for no leader in original partition
	fmt.Println("Checking for no leader in original partition")
	cfg.checkNoLeader()
...
func TestLargePartitionNoConsensusHidden2A(t *testing.T) {
	...
	fmt.Printf("Test (2A): Test Large Parition No Consensus 2A\n")
	fmt.Printf("Basic 1 leader\n")
	leader1 := cfg.checkOneLeader()

	// forming one partition with one leader and 6 nodes
	fmt.Println("Forming partition 2 with leader and 6 nodes")
	partition2 := make(IntSet)
	partition2[leader1] = struct{}{}
	partition2[(leader1+1)%servers] = struct{}{}
	partition2[(leader1+2)%servers] = struct{}{}
	partition2[(leader1+3)%servers] = struct{}{}
	partition2[(leader1+4)%servers] = struct{}{}
	partition2[(leader1+5)%servers] = struct{}{}
	partition2[(leader1+6)%servers] = struct{}{}
	cfg.disconnect_partition(partition2)
